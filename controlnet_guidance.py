#@title Stable Diffusion guidance model
...
class StableDiffusionControlNet(nn.Module):
    def __init__(self, device, fp16, vram_O, sd_version='2.1', hf_key=None,
                 min_train_step_scaler=0.02, max_train_step_scaler=0.98,
        ...
        #                                                cache_dir="/projectnb/ivc-ml/dbash/huggingface_cache/")
        controlnet = ControlNetModel.from_pretrained(controlnet_path, torch_dtype=torch.float16)
        pipe = StableDiffusionControlNetPipeline.from_pretrained(
            model_key, controlnet=controlnet, torch_dtype=precision_t,
            safety_checker=None,
            cache_dir="/projectnb/ivc-ml/dbash/huggingface_cache/")
        ...
        self.min_step = int(self.num_train_timesteps * min_train_step_scaler)
        self.max_step = int(self.num_train_timesteps * max_train_step_scaler)
    ...
    def train_step(self, text_embeddings, pred_rgb, input_rgb, guidance_scale=10, as_latent=False,  grad_scale=None, t=None):
        ...
        # t = torch.tensor(10, dtype=torch.int16)
        ...
        with torch.no_grad():
            ...
            tt = t.repeat((2,))
            input_rgb_x2 = input_rgb.repeat((2, 1, 1, 1))
            # Save input tensors for UNet
            text_embeds_x2 = text_embeddings.repeat_interleave(batch_size, dim=0)

            down_block_res_samples, mid_block_res_sample = self.controlnet(
                    latent_model_input,
                    tt,
                    encoder_hidden_states=text_embeds_x2,
                    controlnet_cond=input_rgb_x2,
                    return_dict=False,
                )
            noise_pred = self.unet(
                latent_model_input, tt, encoder_hidden_states=text_embeds_x2,
                down_block_additional_residuals=[x.to(dtype=self.precision_t) for x in down_block_res_samples],
                mid_block_additional_residual=mid_block_res_sample.to(self.precision_t)).sample
